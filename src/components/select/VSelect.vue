<script lang="ts" setup>
  ...
  const selectedOptions = computed((): VtSelectOption[] => {
    ...
    const allRegisteredArr = Array.from(allRegisteredOptions.value.values());
    const cachedArr = Array.from(selectedOptionsCache.value.values());
    ...
    for (const modelVal of modelArray) {
      // 1. По точному ключу
      const key = getOptionKey(modelVal);
      const foundByKey = allRegisteredOptions.value.get(key);

      if (foundByKey && activeOptionKeys.value.has(key)) {
        selectedOptionsCache.value.set(key, foundByKey);
        result.push(foundByKey);
        continue;
      }

      // 2. По всіх опціях через compareValues (різні reference, різний JSON.stringify)
      const foundByCompare = allRegisteredArr.find(
        opt => activeOptionKeys.value.has(getOptionKey(opt.value)) && compareValues(opt.value, modelVal, props.valueKey)
      );

      if (foundByCompare) {
        selectedOptionsCache.value.set(getOptionKey(foundByCompare.value), foundByCompare);
        result.push(foundByCompare);
        continue;
      }

      // 3. З кешу по ключу
      if (selectedOptionsCache.value.has(key)) {
        result.push(selectedOptionsCache.value.get(key)!);
        continue;
      }

      // 4. З кешу через compareValues
      const foundInCache = cachedArr.find(opt => compareValues(opt.value, modelVal, props.valueKey));

      if (foundInCache) {
        result.push(foundInCache);
        continue;
      }
  ...
  const getLabelFromValue = (value: any): string => {
    ...
    // Спочатку шукаємо в зареєстрованих опціях через compareValues
    const registeredValues = Array.from(allRegisteredOptions.value.values());
    for (const opt of registeredValues) {
      if (compareValues(opt.value, value, props.valueKey)) {
        return opt.label ?? '';
      }
    }

    // Якщо опція ще не зареєстрована — намагаємось витягнути label з самого об'єкта
